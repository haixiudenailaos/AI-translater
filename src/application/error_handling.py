...
def classify_error(error: BaseException) -> ActionableError:
    ...
    if status_code in (401, 403) or any(token in lowered for token in ("api key", "unauthorized", "forbidden", "authentication")):
        return ActionableError(ErrorCategory.AUTHENTICATION, message, "检查 API Key 和账户权限后重试。", False)
    if status_code == 429 or "rate limit" in lowered or "限流" in message:
        return ActionableError(ErrorCategory.RATE_LIMIT, message, "稍后重试，或降低并发和批次大小。", True)
    if status_code in (402, 429) or any(token in lowered for token in ("quota", "balance", "额度", "余额")):
    ...
    if status_code in (404, 410, 503) or "model" in lowered and "available" in lowered:
        return ActionableError(ErrorCategory.MODEL_UNAVAILABLE, message, "更换可用模型后重试失败行。", status_code == 503)
    if isinstance(error, (TimeoutError, ConnectionError)) or any(token in lowered for token in ("timeout", "connect", "network", "连接", "超时")):
        return ActionableError(ErrorCategory.NETWORK, message, "检查网络连接后重试失败行。", True)
    if isinstance(error, OSError):
        return ActionableError(ErrorCategory.LOCAL_IO, message, "检查文件路径、权限和磁盘空间后重试保存。", False)
    if isinstance(error, TranslationRequestError) and "line" in lowered:
        return ActionableError(ErrorCategory.RESPONSE_FORMAT, message, "重试失败行；若仍失败，请更换模型。", True)
