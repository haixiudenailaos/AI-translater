...
严禁重复提示词到翻译内容中，翻译严禁出现错字漏字，错字漏字会被定义为失败，严禁任何失败。"""
    ...
    def save_volc_key(self, api_key: str) -> bool:
        """保存火山引擎API密钥"""
        try:
            # 写入文件（权限600，但在Windows上os.chmod可能不完全生效，尽量设置）
            data = {"ark_api_key": api_key.strip()}
            with open(self.volc_key_file, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            
            # 尝试设置文件权限 (Windows上仅只读/隐藏，这里仅作为意图表达)
            try:
                os.chmod(self.volc_key_file, 0o600)
            except Exception:
                pass
        ...
        if self.volc_key_file.exists():
            try:
                with open(self.volc_key_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    key = data.get("ark_api_key", "").strip()
                    if key:
                        return key
            except Exception:
                pass
