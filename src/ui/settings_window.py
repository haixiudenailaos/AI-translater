...
class SettingsWindow:
    ...
    def create_volc_tab(self, notebook):
        ...
        notebook.add(volc_frame, text="插图翻译设置")

        # 插图翻译说明
        ttk.Label(volc_frame, text="插图翻译提供商:").grid(row=0, column=0, sticky=tk.W, padx=10, pady=10)
        ttk.Label(volc_frame, text="火山引擎 Doubao-SeeDream",
                  font=('TkDefaultFont', 9, 'bold')).grid(row=0, column=1, padx=10, pady=10, sticky=tk.W)

        # 提供商说明
        desc_label = ttk.Label(volc_frame, text="使用火山引擎 Doubao-SeeDream 模型进行插图翻译，需配置API Key",
                              wraplength=450, foreground="#666")
        desc_label.grid(row=1, column=0, columnspan=2, padx=10, pady=(0, 5), sticky=tk.W)

        # 火山引擎 API Key
        ttk.Label(volc_frame, text="火山引擎 API Key:").grid(row=2, column=0, sticky=tk.W, padx=10, pady=10)

        self.volc_key_var = tk.StringVar(value=self.config_manager.get_volc_key())
        key_entry = ttk.Entry(volc_frame, textvariable=self.volc_key_var, show="*", width=35)
        key_entry.grid(row=2, column=1, padx=10, pady=10, sticky=tk.W)

        # 提示
        tip_label = ttk.Label(volc_frame, text="火山引擎API Key用于插图翻译功能",
                             font=('TkDefaultFont', 8), foreground="#888")
        tip_label.grid(row=3, column=1, sticky=tk.W, padx=10, pady=(0, 10))

        # 测试按钮
        test_btn = ttk.Button(volc_frame, text="测试插图翻译连接", command=self.test_volc_connection)
        test_btn.grid(row=4, column=1, sticky=tk.W, padx=10, pady=10)

        # 分隔线
        separator = ttk.Separator(volc_frame, orient=tk.HORIZONTAL)
        separator.grid(row=5, column=0, columnspan=2, sticky=tk.EW, padx=10, pady=10)

        # 图片文字识别翻译区域
        vision_desc = ttk.Label(volc_frame, text="使用视觉模型（VL）识别EPUB图片中的外文文字并翻译为目标语言",
                               wraplength=450, foreground="#666")
        vision_desc.grid(row=6, column=0, columnspan=2, padx=10, pady=(5, 5), sticky=tk.W)

        # 启用复选框
        self.image_text_enabled_var = tk.BooleanVar(
            value=self.app_config.get("image_text_translation_enabled", True)
        )
        enable_check = ttk.Checkbutton(volc_frame, text="启用图片文字识别翻译",
                                       variable=self.image_text_enabled_var)
        enable_check.grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=10, pady=5)

        # 视觉模型（固定）
        ttk.Label(volc_frame, text="视觉模型:").grid(row=8, column=0, sticky=tk.W, padx=10, pady=10)

        vision_model_label = ttk.Label(volc_frame, text="PaddlePaddle/PaddleOCR-VL-1.5",
                                       font=('TkDefaultFont', 9, 'bold'))
        vision_model_label.grid(row=8, column=1, padx=10, pady=10, sticky=tk.W)
    ...
    def test_volc_connection(self):
        ...
        def worker():
            ...
            try:
                ...
                if success:
                    self.window.after(0, lambda: messagebox.showinfo("测试成功", "火山引擎插图翻译服务可用！"))
                else:
                    self.window.after(0, lambda: messagebox.showerror("测试失败", f"连接失败或鉴权错误，请检查 API Key。"))
            except Exception as e:
                self.window.after(0, lambda: messagebox.showerror("测试错误", f"发生异常: {str(e)}"))
    ...
    def save_settings(self):
        ...
        try:
            ...
            new_app_config = {
                ...
                "vision_model_name": "PaddlePaddle/PaddleOCR-VL-1.5",
