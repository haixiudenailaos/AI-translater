...
class TranslatorConfig(BaseModel):
    ...
    external_llm_base_url: str = os.getenv('EXTERNAL_LLM_BASE_URL', 'http://localhost:11434/v1')
    """Base URL of an OpenAI-compatible Chat Completions endpoint."""
    external_llm_api_key: Optional[SecretStr] = None
    """API key for the external endpoint. EXTERNAL_LLM_API_KEY is used when omitted."""
    external_llm_model: str = os.getenv('EXTERNAL_LLM_MODEL', '')
    """Model identifier exposed by the external OpenAI-compatible endpoint."""
    external_llm_context_window: int = Field(
        default=int(os.getenv('EXTERNAL_LLM_CONTEXT_WINDOW', '128000')),
        ge=4096,
        le=4_000_000,
    )
    """Advertised model context window used for conservative prompt budgeting."""
    external_llm_context_pages: int = Field(default=12, ge=0, le=500)
    """Number of prior translated pages supplied as source/translation reference pairs."""
    external_llm_max_output_tokens: int = Field(default=4096, ge=256, le=131072)
    """Maximum output token budget requested from the external model."""
    external_llm_batch_size: int = Field(default=64, ge=1, le=500)
    """Maximum number of OCR text regions translated in one request."""
    external_llm_temperature: float = Field(default=0.2, ge=0.0, le=2.0)
    """Sampling temperature used for translation requests."""
    external_llm_glossary_path: Optional[str] = None
    """Optional pinned glossary. Supports JSON objects or source => target text lines."""
    external_llm_memory_path: Optional[str] = None
    """Internal job memory path used to preserve terminology across batches and resumes."""
    external_llm_json_mode: bool = True
    """Request JSON mode when the compatible endpoint supports it, with automatic fallback."""
    external_llm_retry_attempts: int = Field(default=3, ge=1, le=10)
    """Maximum request or response-format attempts per batch."""
