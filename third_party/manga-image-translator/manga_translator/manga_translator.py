...
class MangaTranslator:
    ...
    def _build_prev_context(
        ...
        """Build bounded prior-page context as approved source/translation pairs."""
        page_limit = self.context_size if page_limit is None else max(0, int(page_limit))
        if page_limit <= 0:
            return ""

        completed_pages = self.all_page_translations or []
        if current_page_index is not None:
            completed_pages = completed_pages[:current_page_index]

        # Each entry is (translated page, original-only page). A translated page maps
        # source text to target text; an original-only page maps region ids to source text.
        available_pages = [(page, None) for page in completed_pages]
        if use_original_text and batch_index is not None and batch_original_texts is not None:
            for index in range(min(batch_index, len(batch_original_texts))):
                original_page = batch_original_texts[index]
                if original_page:
                    available_pages.append((None, original_page))

        def page_has_text(entry):
            translated_page, original_page = entry
            if original_page:
                return any(str(text).strip() for text in original_page.values())
            if not translated_page:
                return False
            return any(str(source).strip() or str(target).strip() for source, target in translated_page.items())

        tail = [entry for entry in available_pages if page_has_text(entry)][-page_limit:]
        if not tail:
            return ""

        lines = []
        for translated_page, original_page in tail:
            if use_original_text:
                source_values = original_page.values() if original_page else translated_page.keys()
                for source in source_values:
                    source = str(source).strip()
                    if source:
                        lines.append(f"SOURCE: {source}")
            elif translated_page:
                for source, target in translated_page.items():
                    source = str(source).strip()
        ...
        if use_original_text:
            header = "Previous source segments for narrative reference only:"
        else:
            header = "Previous approved source/translation pairs. Reuse established terminology:"
        return header + "\n" + "\n".join(numbered)
    ...
    async def _dispatch_with_context(self, config: Config, texts: list[str], ctx: Context):
        ...
        context_pages = self.context_size
        if config.translator.translator == Translator.external_llm:
            context_pages = config.translator.external_llm_context_pages

        done_pages = self.all_page_translations
        if context_pages > 0 and done_pages:
            pages_expected = min(context_pages, len(done_pages))
            non_empty_pages = [
                page for page in done_pages
                if any(sent.strip() for sent in page.values())
            ]
            pages_used = min(context_pages, len(non_empty_pages))
            skipped = pages_expected - pages_used
        else:
            pages_used = skipped = 0

        if context_pages > 0:
            logger.info(f"Context-aware translation enabled with {context_pages} pages of history")

        # 构建上下文字符串
        # Build the context string
        prev_ctx = self._build_prev_context(page_limit=context_pages)

        # 如果是 ChatGPT 或 ChatGPT2Stage 翻译器，则专门处理上下文注入
        # Special handling for ChatGPT and ChatGPT2Stage translators: inject context
        if config.translator.translator in [
            Translator.chatgpt,
            ...
            if config.translator.translator == Translator.chatgpt:
                ...
                translator = get_translator(Translator.external_llm)
            ...
            elif config.translator.translator == Translator.external_llm:
    ...
    async def _batch_translate_texts(self, texts: List[str], config: Config, ctx: Context, batch_contexts: List[Context] = None, page_index: int = None, batch_index: int = None, batch_original_texts: List[dict] = None) -> List[str]:
        ...
        # Context-aware LLM translators need approved history injected explicitly.
        if config.translator.translator in [
            Translator.chatgpt,
            Translator.chatgpt_2stage,
            Translator.external_llm,
            ...
            if context_pages > 0:
                ...
                logger.info(f"Context-aware translation enabled with {context_pages} pages of history using {context_type}")
