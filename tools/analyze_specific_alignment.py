...
def analyze_specific_lines(mapping_dir: str, start_line: int, end_line: int):
    ...
    for entry in target_entries:
        ...
        if entry["original_text"] and entry["translated_text"]:
            ...
            if ratio > 3 or ratio < 0.3:  # 长度差异过大
                issues.append(
                    f"长度差异异常 (原文:{orig_len}字, 译文:{trans_len}字, 比例:{ratio:.2f})"
                )
        ...
        if (
            ...
            and len(entry["translated_text"]) > 20
            and entry["original_text"].startswith("「")
            and entry["original_text"].endswith("」")
            and not (
                entry["translated_text"].startswith('"')
                or entry["translated_text"].startswith("「")
            )
        ):
        ...
        if issues:
            alignment_issues.append(
                {"line_number": entry["line_number"], "key": entry["key"], "issues": issues}
            )
    ...
    if alignment_issues:
        ...
        for issue in alignment_issues:
            print(
                f"  - 第 {issue['line_number']} 行 ({issue['key']}): {', '.join(issue['issues'])}"
            )

        print("\n可能的原因:")
        print("  1. 翻译过程中出现了行号错位")
        print("  2. 批量翻译时API返回的译文顺序与原文不匹配")
        print("  3. 手动编辑时误操作导致内容错位")
        print("  4. 翻译工具的对齐机制存在bug")

        print("\n建议修复方案:")
        print("  1. 检查翻译工具的save_translations方法是否正确按line_number对齐")
        print("  2. 重新翻译这些有问题的行")
        print("  3. 手动修正错位的译文内容")
...
def main():
    ...
    if len(sys.argv) < 4:
        ...
        print(
            '  python analyze_specific_alignment.py "C:/Users/user/Desktop/mapping/日文原版(1)" 630 633'
        )
        sys.exit(1)
