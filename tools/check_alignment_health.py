...
def check_mapping_health(mapping_dir: str):
    ...
    for i, entry in enumerate(entries):
        ...
        if actual != expected:
            discontinuities.append(
                {"position": i, "expected": expected, "actual": actual, "key": entry["key"]}
            )

        # 检查重复
        if i > 0 and entry["line_number"] == entries[i - 1]["line_number"]:
            duplicates.append(
                {"line_number": actual, "keys": [entries[i - 1]["key"], entry["key"]]}
            )
    ...
    if discontinuities:
        ...
        for d in discontinuities[:10]:
            print(
                f"     位置 {d['position']}: 期望 {d['expected']}, 实际 {d['actual']} (键: {d['key']})"
            )
        return False
    else:
